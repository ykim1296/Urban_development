...
### Y: response, L_J2-L_J5: 1-lag to 4-lag temporal
...
### Get the 10 nearest block by each block
nbh.size <- 10
block.nbhs <- t(apply(dBlock[,-1], 1, function(x) which(x %in% sort(x)[2:(nbh.size + 1)]) - 1))

### NEFFECT.PAST.L_J2 - NEFFECT.PAST.L_J5: 1-lag neighborhood effect to 4-lag neighborhood effect
fld.ys.nbh <- list()
k <- 1
for (yr in years) {
...
xgb.fit=xgb.train(
  ...
  #nthreads=1,
  early_stopping_rounds=20,
  watchlist=list(val1=train,val2=test),  # To avoid overfitting
  verbose=0
)
